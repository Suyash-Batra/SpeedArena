        ...
        if (cname != null && p != null) {
            try {
                ...
                if (rs.next()) {
                    ...
                    int streak = rs.getInt(7);
                    session.setAttribute("streak", streak);
                    session.setAttribute("userid", userid);
                    session.setAttribute("username", cname);
                    PreparedStatement sp = con.prepareStatement("UPDATE users SET streak = CASE WHEN last_login IS NULL THEN 1 WHEN DATEDIFF(CURDATE(), last_login) = 1 THEN streak + 1 WHEN DATEDIFF(CURDATE(), last_login) > 1 THEN 1 ELSE streak END, last_login = CURDATE() WHERE user_id = ?");
                    sp.setInt(1,userid);
                    sp.executeUpdate();
